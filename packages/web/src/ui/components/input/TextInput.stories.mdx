...
<Meta title="Component Library/Input/TextInput" component={TextInput}/>
...
But then we can take it further and add the `notEmpty` paremeter. Although this renders the same thing, if there is nothing in the input, it will have a red border! You can also pass `errorWait` as true to make it only turn red once the user has altered the value in some way.
...
You can add parameters like `placeholder`, `autocomplete`, and `mode`
...
You can also use the `errorWait` property to tell the error border and label to only appear once the user has interacted in some way.
